...
class SignerScreen extends StatefulWidget {
  ...
  const SignerScreen({required this.signer});

  @override
  _SignerScreenState createState() => _SignerScreenState();
}

class _SignerScreenState extends State<SignerScreen> {
  List<VideoItem> signerVideos = [];
  bool isLoading = true;
  ...
  Future<void> loadSignerVideos() async {
    final String jsonString =
    await rootBundle.loadString('assets/sign_videos_metadata.json');
    final List<dynamic> jsonData = json.decode(jsonString);

    final List<VideoItem> allVideos =
    jsonData.map((item) => VideoItem.fromJson(item)).toList();

    setState(() {
      signerVideos =
          allVideos.where((video) => video.signer == widget.signer).toList();
      isLoading = false;
    });
  }
                ...
                child: isLoading
                  ...
                  child: GridView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final video = signerVideos[index];
                                ...
                                child: IconButton(
                                  icon: Icon(
                                    FavoritesManager.isFavorite(
                                        ...
                                        ? Icons.star
                                        : Icons.star_border,
                                    color: FavoritesManager.isFavorite(
                                        video.videoPath)
                                        ? Colors.amber
                                        : Colors.grey,
                                  ),
